...
const StatCard: React.FC<StatCardProps> = ({ icon, label, value, color, secondaryValue }) => {
    return (
        <div className={`relative overflow-hidden bg-[var(--bg-glass)] backdrop-blur-lg border border-[var(--border-color)] rounded-xl p-5 transform hover:-translate-y-1.5 transition-transform duration-300 group`}>
            <div className={`absolute -top-1/2 -left-1/2 w-full h-full bg-gradient-to-br from-transparent via-${color}-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
            <div className={`absolute inset-0 rounded-xl border-2 border-transparent group-hover:border-${color}-500 transition-all duration-300`}></div>
            <div className="flex items-center gap-5 relative z-10">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 bg-${color}-500/20 text-${color}-300 group-hover:shadow-${color} transition-shadow duration-300`}>
                    {icon}
                </div>
                <div className="flex-grow">
                     <div className="flex items-baseline gap-2">
                         <div className="text-4xl font-bold text-slate-100"><AnimatedNumber value={value} /></div>
...
const StatsDashboard: React.FC<StatsDashboardProps> = ({ students, selectedDate }) => {
  const stats = useMemo(() => {
    ...
    const studentsWithSiblings = students.filter(s => s.hasSiblings === 'نعم').length;
  ...
  return (
    <div style={{ animationDelay: '200ms' }} className="animate-fade-in-down grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <StatCard
        ...
        color="purple"
        icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21v-1a6 6 0 00-1.781-4.121M12 12a4 4 0 110-8 4 4 0 010 8z" /></svg>}
      />
    </div>
  );
};
